...
public class InQueueConsumer extends QueueConsumer {
	...
	public InQueueConsumer(BlockingQueue<Packet> outQueue, Properties conf,
			...
			PacketReceiver packetReceiver, ServiceDiscoveryRegistry discovery, final XMPPConnection connection) {
		super(inQueue);
		...
		Thread t = new Thread() {
			public void run() {
				...
				try {
					...
					OperationsFactory operations = new OperationsFactory(discovery, connection, channelManager);

					if (p instanceof IQ) {
						new IQProcessor(outQueue, conf, channelManager, operations)
								.process((IQ) p);
